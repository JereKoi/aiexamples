...
int main() {
    ...
    pixelgridworld::Window window("AStar demo", 512);
    ...
    auto getHCost = [](SearchNode* node, core::Vector endState) -> float {
        // Nyt, Euclidian distance (voi olla myös manhattan distance tai hamming distance)
    ...
    while(window.shouldClose() == false){
        ...
        auto openList = astar(getNextLegalStates, getFCost, getGCost, state, goalState);
        if (openList.size() == 0) {
